...
- `apps/<app>/release.json` as the latest desired or staged release pointer
- `apps/<app>/active_release.json` as the last successfully applied release
...
./cli/ship release show <app> active
...
successfully applies, Ophelia removes stale generated support files that are no
longer referenced by the active release bundle.
...
- `traffic_switching`: explicit metadata that rollback is a generated-file
  restore; traffic provider intent lives in `ship app traffic` receipts and is
  not executed by release rollback
