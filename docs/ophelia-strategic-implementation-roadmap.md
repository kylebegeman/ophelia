...
Status: selected next-stage roadmap
...
- Add a shared `digest` helper that can be attached to plan and receipt
  envelopes.
- Add operation-specific digest builders for deploy, app factory, traffic,
  workflow, backup/restore, GitHub provisioning, and observability.
