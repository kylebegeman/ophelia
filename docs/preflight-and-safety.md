...
- target health verification is read-only, requires an explicit
  `--target-health-url` and `--run-target-health`, rejects credential-bearing
  URLs, and is covered by the matching confirmation token
...
- Cloudflare DNS uses `api_token_env`; token values are not stored in plans or
  receipts, and apply never deletes DNS records
- live Caddy reload through the file provider requires `reload: true`,
  `allow_reload: true`, the matching plan token, and the normal provider
  execution gates
