...
Import apply, restore drill apply, cutover apply, and isolation apply are not
exposed as mutating action descriptors in this planning pass. Export create is
exposed as `app.export.create`, but only writes local metadata/runtime bundle
artifacts, a deterministic `.tar` archive, and receipts after a matching dry-run
confirmation.
