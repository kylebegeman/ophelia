...
bundle directory containing `manifest.json`, `source-host.json`, sanitized
runtime files, checksums, and JSON receipts. It does not run live Postgres dumps,
Redis exports, volume archives, Caddy changes, Docker operations, or SSH.
Those data-export modes remain future work and must stay behind confirmation
tokens from matching plans.
