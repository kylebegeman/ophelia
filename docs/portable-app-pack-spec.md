...
Export create is implemented for metadata and redacted runtime files. It writes a
bundle directory and a deterministic `.tar` archive containing `manifest.json`,
`source-host.json`, sanitized runtime files, checksums, and JSON receipts. It
does not run live Postgres dumps, Redis exports, volume archives, Caddy changes,
Docker operations, or SSH. `.tar.zst` packaging and live data-export modes
remain future work and must stay behind confirmation tokens from matching plans.
...
Import apply remains future work and must require the confirmation token from a
matching import plan.
...
A restore drill should prove that the exported data can be restored into an
isolated target service without damaging the live app. The current command
surface implements the plan form only; apply remains future work and must
require the confirmation token from the plan.
...
Cutover apply should only run after a successful import and verification on the
target host. The current command surface implements the plan form only; apply
remains future work and must require the confirmation token from the plan.
...
the target per-app network shape. It does not rewrite Compose output yet.
