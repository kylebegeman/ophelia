...
http(s), cannot contain credentials, and can set timeout/expected status with
`--target-health-timeout` and `--target-health-expect-status`.
...
requests it. Reload requires both `reload: true` and `allow_reload: true`:
