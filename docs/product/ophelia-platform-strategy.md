...
architecture, and implementation roadmap originally performed against Ophelia
0.4.4 at commit `e576d7d`. The direction was accepted on 2026-07-19 and is the
architectural north star for work on `next`.

Acceptance of the direction does not by itself authorize production mutation,
host migration, data movement, DNS changes, or deletion. The active
[Roadmap](../ROADMAP.md) remains the delivery record for converting the
direction into reviewed, verified releases.
