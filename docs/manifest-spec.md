...
`ship app export create` is confirmation-gated and currently writes metadata,
redacted runtime files, checksums, receipts, and a deterministic `.tar` archive
of that redacted bundle only. Live database dumps, live volume archives,
`.tar.zst` packaging, and import apply remain future work and must use
confirmation tokens from matching plans.
