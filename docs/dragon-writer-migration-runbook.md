...
mutate Caddy or DNS. The current traffic apply command records DNS/Caddy
provider intent and writes a checkpoint receipt only; it does not mutate
providers until an explicit provider backend is implemented.
