...
These commands must not mutate production state. Import apply, restore drill
apply, cutover apply, traffic apply, live data export, and DNS changes require
separate confirmed steps.

The confirmed export step writes metadata, redacted runtime files, local
static/volume archives for declared local sources, checksums, receipts, and a
`.tar` fallback archive of that redacted bundle. When `zstd` is available, it
also writes the planned `.tar.zst` archive:

```bash
./cli/ship app export create dragon-writer --environment production --confirm <token> --json
```

It writes local export artifacts and receipts only. It does not mutate Caddy,
mutate DNS, delete source data, or SSH to a host. Pass `--include-postgres` on
both export plan and export create to run the allowlisted read-only `pg_dump`
path with redacted receipts.

When the matching plans are reviewed, the implemented confirmed follow-up
commands are:

```bash
./cli/ship app export create dragon-writer --environment production --confirm <token> --json
./cli/ship app import apply ./exports/dragon-writer.production.export.tar --confirm <token> --json
...
The current cutover apply command writes a checkpoint receipt only. It does not
mutate Caddy or DNS. Traffic apply records DNS/Caddy provider intent and writes
a checkpoint receipt by default. Traffic plan can run a read-only target health
gate before apply. File-backed provider execution is available when the
matching plan/apply use `--execute-provider-mutation`,
`--provider-config`, and config-level `allow_mutation: true`; it writes local
DNS record JSON and staged Caddy files. Live Caddy reload additionally requires
`reload: true`, `allow_reload: true`, and `sites_dir` matching
`<runtime_root>/caddy/sites.d` in provider config. Target health URLs must be
http(s) URLs without credentials, query strings, or fragments. External DNS API
changes are supported through the Cloudflare DNS provider when the token is
referenced by `api_token_env`; token values are not written to receipts, and TTL
must be automatic `1` or between `30` and `86400` seconds.
Traffic rollback can restore previous provider state captured by a traffic apply
receipt and blocks deletion-only rollback cases.
...
## Ophelia Features Covered Before Execution

- explicit data contracts in manifests
- source inventory and host capability receipts
- opt-in Postgres dump create with redacted receipts
- local volume/static archive create and listability validation
- export bundle with checksums
- import preview and isolated rehearsal apply
- restore drill artifact/listability receipt
- cutover plan and checkpoint apply receipts
- traffic plan and checkpoint apply receipts
- route/domain conflict scan and DNS checklist
