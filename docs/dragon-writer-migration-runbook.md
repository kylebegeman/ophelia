...
If Dragon Writer stays on a shared Postgres foundation, use
`shared-postgres-database` instead of `app-postgres`, but the export/import
contract must still be explicit.
