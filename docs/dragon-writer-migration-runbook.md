...
apply, cutover apply, live data export, and DNS changes remain separate
confirmed future steps.

The currently implemented confirmed export step is limited to metadata and
redacted runtime files, plus a `.tar` archive of that redacted bundle:

```bash
./cli/ship app export create dragon-writer --environment production --confirm <token> --json
```

It writes local export artifacts and receipts only. It does not run live
database dumps, archive uploads, mutate Caddy, mutate DNS, or SSH to a host.
