...
```bash
./cli/ship app cutover plan demo-service --from spaceship --to ovh --json
./cli/ship app cutover apply demo-service --from spaceship --to ovh --confirm <token>
```
...
Demo Service, Lumen, Stillup, and SaaS apps can live on the same host without
accidental coupling.
