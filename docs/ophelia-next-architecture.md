...
inventory source
  ...
  -> traffic plan
  -> target health gate
  -> traffic apply checkpoint
  -> optional file-backed DNS/Caddy provider execution with explicit config
  -> traffic rollback plan is available from the apply receipt
  -> monitor
...
- `ship app traffic plan|apply` records production traffic automation intent,
  DNS/Caddy provider execution intent, readiness gates, rollback notes,
  target health checks, checkpoint receipts, gated file/Caddy execution, and
  gated Cloudflare DNS updates
