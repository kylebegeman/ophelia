...
inventory source
  ...
  -> provider-backed DNS/Caddy mutation when implemented
  -> monitor
...
- `ship app traffic plan|apply` records production traffic automation intent,
  DNS/Caddy provider placeholders, readiness gates, rollback notes, and
  checkpoint receipts without mutating providers
