...
Current Ophelia attaches app services to both `ophelia-edge` and
`ophelia-internal`. That is workable for the first generation, but the next
...
- Add Postgres dump support for declared app databases.
- Add volume/static asset archive support.
- Add checksums for exported artifacts.
- Add restore preview and restore drill receipts.
...
- Add import preview from an export bundle.
- Add import apply for non-production rehearsal targets.
- Add target health verification and data verification hooks.
