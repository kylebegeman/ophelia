...
class _FakeStaticBackend:
    ...
    def verify(
        ...
        return _verification(revision, passed, "candidate")
    ...
    def deactivate(self, failed_candidate: RuntimeHandle) -> TrafficActivationResult:
        ...
        self.state.restore_count += 1
        return self._activation_result(
            failed_candidate.revision_digest, _digest("absent")
        )

    def _activation_result(
        self, previous_digest: str | None, active_digest: str
    ) -> TrafficActivationResult:
        evidence = _digest(
...
def _verification(
    revision: Revision, passed: bool, name: str
) -> VerificationResult:
...
def _bundle(
    ...
    *,
    ...
    artifact_ref = RevisionArtifactRef(
        revision_id=revision.revision_id,
        revision_digest=revision.content_digest(),
        relative_root=f"operations/{suffix}/candidate",
        artifact_digest=artifact_digest,
    )
