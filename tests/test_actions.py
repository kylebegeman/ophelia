...
class ActionTests(unittest.TestCase):
    ...
    def test_completion_callback_failure_redacts_url_secrets(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            manifest_path = root / "app.ophelia.yml"
            runtime_root = root / "runtime"
            ...
            os.environ["OPHELIA_ACTION_CONFIG"] = str(config_path)
            try:
                with mock.patch("ophelia.actions.request.urlopen", side_effect=OSError("boom")):
                    result = run_job(
                        "manifest.validate",
                        {
                            "manifest_path": str(manifest_path),
        ...
        self.assertEqual("succeeded", result.job["state"])
        self.assertIn("https://<redacted>@example.com/cb?<redacted>#<redacted>", blob)
