...
def default_registry_path() -> Path:
    ...
    return Path(override) if override else DEFAULT_APP_REGISTRY
...
def _entry(raw: Dict[str, object]) -> AppRegistryEntry:
    health_urls = []
    for item in raw.get("health_urls", []) or []:
    ...
    return AppRegistryEntry(
        ...
        compose_project=str(raw["compose_project"]),
        root_path=str(raw["root_path"]),
        domains=[str(item) for item in raw.get("domains", [])],
        caddy_site_file=str(raw["caddy_site_file"]),
        container_names=[str(item) for item in raw.get("container_names", [])],
        health_urls=health_urls,
        public_docker_network=str(raw.get("public_docker_network") or "ophelia-edge"),
