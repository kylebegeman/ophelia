...
def effective_verification_policy(
    ...
    return {
        "attempts": max(1, int(attempts if attempts is not None else base.attempts)),
        "interval": max(0.0, float(resolved_interval if resolved_interval is not None else base.interval)),
        "timeout": max(0.1, float(timeout if timeout is not None else base.timeout)),
