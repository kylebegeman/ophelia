...
def traffic_plan(
    ...
    if execute_provider_mutation:
        warnings.append(
            schema_issue(
                ...
                "Traffic apply will write configured provider artifacts after confirmation; live reloads and external DNS APIs remain out of scope for file providers.",
            )
        )
    else:
        warnings.append(
    ...
    environment_arg = f"--environment {resolved_environment} " if resolved_environment in {"dev", "staging", "production"} else ""
    provider_config_arg = f"--provider-config {provider_config.expanduser()} " if provider_config is not None else ""
    execute_arg = "--execute-provider-mutation " if execute_provider_mutation else ""
    exact_command = (
        f"ship app traffic apply {app} --from {source_host} --to {target_host} "
        f"--target-origin {target_origin} {environment_arg}"
        f"--dns-provider {dns_provider} --caddy-provider {caddy_provider} --ttl {ttl} "
        f"{provider_config_arg}{execute_arg}--confirm {token}"
    )
    return plan_envelope(
        "app.traffic.plan",
        app,
        ...
        blockers=blockers,
        warnings=warnings,
        checks=_traffic_checks(readiness, traffic_changes, provider_execution),
...
def traffic_apply(
    ...
    receipt = receipt_envelope(
        ...
        traffic_mutation_performed=provider_mutation_succeeded,
        provider_mutation_performed=provider_mutation_succeeded,
        inputs_redacted=True,
        secrets_redacted=True,
    )
        ...
        changes.append(
            {
                ...
                "caddy": {
                    ...
                    "mutation_supported": False,
                    "planned_action": "stage target route and reload only when a provider backend is configured",
                },
                "rollback": {
                    "source_owner": source_host,
...
def _traffic_checks(readiness: Dict[str, object], traffic_changes: List[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    return [
        ...
        {"name": "provider_mutation", "ok": True, "message": "provider mutation disabled in this checkpoint phase"},
    ]
