...
def export_plan(
    ...
    if manifest is None or resolution.manifest_path is None:
        ...
        _merge_plan_envelope(
            ...
            risk="high",
            exact_command=f"ship app export create {app} --environment {environment or 'unknown'} --confirm {plan['confirmation_token']}",
        )
        return plan
    ...
    _merge_plan_envelope(
        ...
        risk="high",
        exact_command=f"ship app export create {manifest.app} --environment {resolved_environment} --confirm {plan['confirmation_token']}",
    )
    return plan
