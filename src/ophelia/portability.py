...
def traffic_plan(
    ...
    if dns_provider != "manual":
        blockers.append(schema_issue("dns_provider_unsupported", f"DNS provider `{dns_provider}` is not implemented yet.", "dns_provider"))
    if caddy_provider != "manual":
        blockers.append(schema_issue("caddy_provider_unsupported", f"Caddy provider `{caddy_provider}` is not implemented yet.", "caddy_provider"))
    if ttl <= 0:
    ...
    warnings.append(
        schema_issue(
            "traffic_apply_checkpoint_only",
            "Traffic apply currently writes an approval checkpoint receipt only; Caddy and DNS providers are not mutated.",
        )
    )
    token = _token(
        "app.traffic.apply",
    ...
    exact_command = (
        f"ship app traffic apply {app} --from {source_host} --to {target_host} "
        f"--target-origin {target_origin} --environment {resolved_environment} "
        f"--dns-provider {dns_provider} --caddy-provider {caddy_provider} --ttl {ttl} --confirm {token}"
    )
    return plan_envelope(
        "app.traffic.plan",
        app,
        ...
        blockers=blockers,
        warnings=warnings,
        checks=_traffic_checks(readiness, traffic_changes),
        artifacts=[],
        confirmation_required=True,
        confirmation_token=token,
        ...
        rollback={
            "available": True,
            "note": "Before provider mutation, rollback is canceling this traffic plan. After real provider mutation, rollback must restore prior DNS/Caddy owner records from the receipt.",
        },
        apply_supported=True,
        traffic_mutation_supported=False,
        provider_mutation_supported=False,
    )
...
def traffic_apply(
    ...
    receipt = receipt_envelope(
        ...
        checks=[
            ...
            {"name": "active_runtime_unchanged", "ok": True, "message": "No active runtime files were modified."},
            {"name": "provider_mutation", "ok": True, "message": "Caddy and DNS providers were not mutated by this checkpoint apply."},
        ],
        rollback={
            "available": True,
            "note": "No traffic provider mutation was performed. Rollback is canceling this traffic checkpoint before manual or future automated traffic movement.",
        },
        plan_operation_id=plan.get("operation_id") if isinstance(plan.get("operation_id"), str) else None,
        warnings=warnings,
        source_host=source_host,
        ...
        traffic_mutation_performed=False,
