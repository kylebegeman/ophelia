    ...
    * raw stdout/stderr is not stored in receipts, only small parsed summaries.
    ...
    for raw_node in raw_nodes:
        ...
        if not command_tokens:
            node["status"] = "blocked"
            node["blockers"] = [issue("workflow_missing_command", f"Node '{node_id}' has no runnable command.")]
            blockers.extend(node["blockers"])
            status_by_id[node_id] = "blocked"
            run_nodes.append(node)
            continue
        if command_has_shell_metacharacters(command_tokens):
            node["status"] = "blocked"
            node["blockers"] = [
                issue("workflow_unsafe_command", f"Node '{node_id}' command contains a shell metacharacter.")
            ]
            blockers.extend(node["blockers"])
            status_by_id[node_id] = "blocked"
            run_nodes.append(node)
