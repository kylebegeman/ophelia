...
_EXAMPLES_BY_OPERATION: Dict[str, List[str]] = {
    ...
    "runtime.drift": ["ship drift examples/service-app.ophelia.yml --json"],
    "validate": ["ship validate examples/service-app.ophelia.yml --json"],
    "render": ["ship render examples/portfolio.ophelia.yml --output-dir ./build/portfolio --json"],
    "manifest.validate": ["ship validate examples/service-app.ophelia.yml --json"],
    "manifest.explain": ["ship explain examples/service-app.ophelia.yml --json"],
    ...
    "restore.plan": ["ship restore plan demo-service <backup-id> --json"],
    "restore.apply": ["ship restore apply demo-service <backup-id> --confirm <token> --json"],
    "host.inventory": ["ship host inventory --json"],
    "host.readiness": ["ship host readiness local --json", "ship host readiness --json"],
    "app.readiness": ["ship app readiness demo-service --environment production --json"],
    "app.adoption.plan": [
    ...
    "app.placement.plan": ["ship app placement demo-service --environment production --from source-host --to target-host --json"],
    "app.runbook": ["ship app runbook demo-service --environment production"],
    "app.export.plan": ["ship app export plan demo-service --environment production --json"],
    "app.export.create": ["ship app export create demo-service --environment production --confirm <token> --json"],
    "app.import.plan": ["ship app import plan ./exports/demo-service.production.export/manifest.json --json"],
    "app.import.apply": ["ship app import apply ./exports/demo-service.production.export/manifest.json --confirm <token> --json"],
    "app.restore-drill.plan": ["ship app restore-drill plan demo-service --source ./exports/demo-service.production.export.tar --json"],
    "app.restore-drill.apply": ["ship app restore-drill apply demo-service --source ./exports/demo-service.production.export.tar --confirm <token> --json"],
    "app.cutover.plan": ["ship app cutover plan demo-service --from source-host --to target-host --environment production --json"],
    "app.cutover.apply": ["ship app cutover apply demo-service --from source-host --to target-host --environment production --confirm <token> --json"],
    "app.traffic.plan": ["ship app traffic plan demo-service --from source-host --to target-host --target-origin demo-service-target.example.net --environment production --json"],
    "app.traffic.apply": ["ship app traffic apply demo-service --from source-host --to target-host --target-origin demo-service-target.example.net --environment production --confirm <token> --json"],
    "app.traffic.rollback.plan": ["ship app traffic rollback plan demo-service --receipt latest:app.traffic.apply --environment production --json"],
    "app.traffic.rollback.apply": ["ship app traffic rollback apply demo-service --receipt latest:app.traffic.apply --environment production --confirm <token> --json"],
    "app.isolation.plan": ["ship app isolation plan demo-service --environment production --json"],
    ...
    "receipts.list": ["ship receipts list --app demo-service --json"],
    "receipts.show": ["ship receipts show latest:demo-service --json", "ship receipts show <receipt-id-prefix> --json"],
    "receipts.timeline": ["ship receipts timeline --app demo-service --json"],
    ...
    "state.query.receipts": ["ship state query receipts --app demo-service --json", "ship state query receipts --ref latest:deploy.apply --json"],
    "workflow.plan": [
        "ship workflow plan move-app --app demo-service --from source-host --to target-host --environment production --json",
        "ship workflow plan incident-triage --app demo-service --environment production --json",
    ],
    "workflow.preview": ["ship workflow run latest:demo-service --preview --set MANIFEST_PATH=manifests/demo-service.ophelia.yml --json"],
    "workflow.run": ["ship workflow run latest:demo-service --set MANIFEST_PATH=manifests/demo-service.ophelia.yml --confirm-node export-create=<token> --json"],
    "workflow.pause": ["ship workflow pause latest:demo-service --json"],
    "workflow.resume": ["ship workflow resume latest:demo-service --set MANIFEST_PATH=manifests/demo-service.ophelia.yml --confirm-node export-create=<token> --json"],
    "workflow.cancel": ["ship workflow cancel latest:demo-service --json"],
    "workflow.show": ["ship workflow show latest:demo-service --json"],
    ...
    "observability.plan": ["ship observability plan --app demo-service --environment production --json"],
    "observability.status": ["ship observability status --app demo-service --environment production --json"],
    "observability.export": ["ship observability export --app demo-service --environment production --json"],
    ...
    "secrets.audit": ["ship secrets audit demo-service --environment production --json"],
    "secrets.providers": ["ship secrets providers demo-service --environment production --json"],
    "policy.evaluate": ["ship policy evaluate --operation app.traffic.apply --app demo-service --environment production --json"],
}
