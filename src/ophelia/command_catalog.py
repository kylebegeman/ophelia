...
_EXAMPLES_BY_OPERATION: Dict[str, List[str]] = {
    ...
    "app.traffic.rollback.plan": ["ship app traffic rollback plan dragon-writer --receipt <traffic-receipt-id> --environment production --json"],
    "app.traffic.rollback.apply": ["ship app traffic rollback apply dragon-writer --receipt <traffic-receipt-id> --environment production --confirm <token> --json"],
    ...
    "receipts.show": ["ship receipts show <receipt-id> --json"],
    ...
    "state.query.receipts": ["ship state query receipts --app dragon-writer --json"],
    "workflow.plan": ["ship workflow plan move-app --app dragon-writer --from spaceship --to ovh --environment production --json"],
    "workflow.run": ["ship workflow run <workflow-id> --set MANIFEST_PATH=manifests/dragon-writer.ophelia.yml --json"],
    "workflow.show": ["ship workflow show <workflow-id> --json"],
