...
def _build_remote_stage_script(
    ...
    if plan:
        ...
        lines.extend(_build_stage_lines(manifest))
...
def _build_stage_lines(manifest: Manifest) -> List[str]:
    caddy_target = f"$REMOTE_RUNTIME_ROOT/caddy/sites.d/{manifest.app}.caddy"
    caddy_global_target = f"$REMOTE_RUNTIME_ROOT/caddy/global.d/{manifest.app}.caddy"
    return [
        'mkdir -p "$APP_ROOT/caddy"',
        f"CADDY_TARGET={shlex.quote(caddy_target)}",
        f"CADDY_GLOBAL_TARGET={shlex.quote(caddy_global_target)}",
