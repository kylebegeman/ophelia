...
def image_lock_plan(
    ...
    *,
    ...
    changes = [
        ...
        for entry in locked
        if entry.get("locked_image") and entry.get("locked_image") != entry.get("image")
    ]
    apply_input = {
...
def _image_targets(manifest: Manifest) -> List[Dict[str, object]]:
    ...
    if manifest.image:
        targets.append({"path": "image", "service": "default", "image": manifest.image})
    for service_name, service in sorted(manifest.services.items()):
        if service.image:
            targets.append({"path": f"services.{service_name}.image", "service": service_name, "image": service.image})
        elif manifest.image:
            targets.append({"path": f"services.{service_name}.image", "service": service_name, "image": manifest.image, "inherited": True})
    return targets
...
def _resolve_with_buildx(image: str, *, timeout: float) -> Dict[str, object]:
    ...
    match = re.search(r"(?im)^\s*Digest:\s*(sha256:[0-9a-f]{32,})\s*$", result.stdout)
    if not match:
...
def _resolve_with_docker_manifest(image: str, *, timeout: float) -> Dict[str, object]:
    ...
    match = re.search(r"(sha256:[0-9a-f]{32,})", result.stdout)
    if match:
