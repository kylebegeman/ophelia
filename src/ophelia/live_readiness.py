...
def live_readiness_report(
    *,
    ...
    requested by flags.
    """
    restore_docker_status = _disable_docker_status_if_needed(check_docker)
...
def _disable_docker_status_if_needed(check_docker: bool) -> tuple[bool, Optional[str]]:
    if check_docker:
        return (False, None)
...
def _restore_docker_status(state: tuple[bool, Optional[str]]) -> None:
